...
Use Sonnet model. Pass the rendered JSON input as the only message.
...
### 3.3 Coder [Haiku for Tier 1 / Sonnet for Tier 2-3]
...
Use Haiku model for Tier 1. Use Sonnet model for Tier 2 and Tier 3.
...
HARD LIMITS — enforce these absolutely:
- Maximum 2 WebSearch calls total. Stop after 2 regardless of results.
- Maximum 3 WebFetch attempts total. Stop after 3 regardless of outcome.
- If no suitable image is found within these limits, return reference_path: null immediately. Do not search further.

PROCEDURE:
1. Use WebSearch (up to 2 calls) targeting the specific subject by name — search for the exact character, logo, or object, not a generic description. Prefer PNG/transparent background sources, fan wikis, or official art pages.
2. From the results, pick the single best candidate against these criteria:
   a. Isolated subject on a clean or transparent background
   b. Silhouette matches the subject
   c. Resolution >= 256px on the short side
3. Use WebFetch to verify the chosen image loads (counts toward your 3 WebFetch limit). If it fails, try one alternative. If that fails too, return null.
4. Download the chosen image to: agents/.forge_staging/${generation_id}/ref_${slot}.png (use Bash with curl or wget)
...
Never return a path you have not actually downloaded. Never exceed the search limits above.
```

Use Sonnet model. Provide tools: WebSearch, WebFetch, Bash, Read.
